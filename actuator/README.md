...
| `max_torque_nm`    | double[8] | `[1.0]*8`   | **Per-motor** (motor 1..8 order) hard safety ceiling for `set_motor_torque` — motor `i`'s requested torque is clamped to `[-max_torque_nm[i-1], max_torque_nm[i-1]]` server-side, regardless of what the client asks for. Was a single shared double — changed 2026-08-04 after real data (`PPO_5000000_stand_policy_torque_v8_2.csv`, uniform `1.0`) showed the 4 thigh motors (1,4,5,8) pinned at the ceiling 99.7% of the run (genuinely underpowered) while the 4 calf motors (2,3,6,7) swung 150-200+ degrees at that SAME limit (a slipping foot, nothing checking the leg) — one shared number can't serve both. A motor with no entry (array shorter than 8) falls back to a conservative `1.0 N·m` and logs an error — never silently unclamped. `dog_gym` sim training itself used `±20 N·m` — raise deliberately per-motor, only after confirming `real_velocity_deg_s` stays reasonable in a logged CSV at the current values. Example: `ros2 param set /motor_test max_torque_nm "[2.0, 1.0, 1.0, 2.0, 2.0, 1.0, 1.0, 2.0]"` (thighs 2.0, calves 1.0). |
...
| `torque_kd_gain`   | double | `0.2`          | Velocity damping for `set_motor_torque` (N·m per rad/s of OUTPUT-shaft velocity, converted to rotor-side internally like `position_kp`/`position_kd`) — `tau_actual = tau_requested - torque_kd_gain*qvel`, a physical brake proportional to velocity, applied every tick regardless of what the client sends. `kp` stays `0` — this doesn't track a target angle, it's still genuine torque control. Added 2026-08-04 after a real breakaway event (static friction held a thigh motionless under climbing torque, then it broke free with nothing resisting velocity and ran away to 1682 deg/s). **MUST match `dog_gym`'s `TORQUE_KD_GAIN` constant exactly** (same units) or sim training and real deployment disagree about how much free braking exists. Untuned placeholder — needs real empirical tuning. |

**Node name depends on how this is launched** — the examples below use the
default `/motor_test` (plain `ros2 run actuator basic_control`), but
`ros2 launch dog_deploy hardware_bringup.launch.py` (the normal way to
bring this up alongside `dog_imu` for `policy_node`) renames it to
`/actuator`. Check `ros2 node list` and substitute whichever name is
actually running before any `ros2 param set`/`ros2 service call` below.
