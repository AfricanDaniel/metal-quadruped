...
# --actuator-type torque_belt (2026-08-05, user request). SEPARATE file from
...
# 10% of the sampled value). WIDENED 2026-08-10 (was 0.3-0.8): a real-
# hardware run showed feet slipping during the backward-drag propulsion
# phase, i.e. the ground is more slippery than anticipated -- there's no real
...
# external push force on the torso (2026-08-09, following a
# single-leg-lift isolation test finding).
...
# WIDENED 2026-08-10 (was 5.0-20.0N): deploying
# PPO_5000000_walk_position_obshistory_home_and_stand_v2 on
# real hardware showed tilting far beyond what sim's push
...
# NEW 2026-08-10, same round of fixes: the OLD mechanism applied force only at
# the torso's CoM (data.xfrc_applied[...,0:3], no torque component) --
# any tilting it produced came ENTIRELY from the stance/support
# geometry's leverage, not a direct rotational push -- an inherently weak
# tilt-inducer compared to a real shove or
...
# report: a known machining error means the back legs are mounted
# closer to center). The MJCF models leg_c_thigh/leg_d_thigh (back-right/
...
# all. (4) the true offset was also directly confirmed to be "impossible
# to measure" precisely (beyond the rough caliper estimate) -- concluded
# a HARD-CODED mean is therefore the wrong tool
...
# mean. Range chosen because the
# caliper estimate (~10-12mm) was itself uncertain, so [5mm, 15mm] straddles it
# with real spread rather than treating it as ground truth.
#
# Sim isolation testing was separately found (during the same
# investigation, via a "quasi-static test") to NOT be a valid
...
# user request to add domain randomization to the center of mass of the
# overall robot).
...
#
# XY (horizontal) vs Z (vertical) given SEPARATE ranges -- an
# extension on the first draft (which left Z at default): XY shift
...
# _STANDING above (2026-08-11, following the decision to
# revert EMA and implement two reward-based fixes instead) --
...
#
# RAISED -1.0 -> -3.0 (2026-08-11, same round of investigation as
# WALK_TROT_SYMMETRY_
# WEIGHT's own increase below, prompted by "v24 plateauing"). Measured directly on PPO_3000000_walk_
...
# rad), not decreased -- the opposite of this term's intent. Diagnosis:
# the policy found it mathematically worthwhile to eat the -1.0
...
# end of a suggested -3.0/-5.0 range) as the next step rather
...
#
# REVERSED DIRECTION -3.0 -> -0.01 (2026-08-11, during the "the triad
# already exists" round of investigation -- externally
...
# own raw (pre-dt-scaling) value directly, which was the basis for this
# recommendation.
WALK_ACTION_RATE_PENALTY_WEIGHT = -0.01

# WALK-only -- see height_sag_penalty's comment in _compute_reward_walk()
# for the full mechanism/reasoning (2026-08-11, same round of fixes as
# WALK_ACTION_RATE_PENALTY_WEIGHT above).
...
# following investigation into "why the reward plateaus" -- the
# recommendation was to add a roll_penalty
...
# level.
#
# RAISED 3.0 -> 5.0 (2026-08-17, same round of roll_penalty
# tuning): direct rollout measurement of PPO_28500000_
...
# already meaningfully suppressing forward credit across the identified
# 15-20deg "danger zone":
# exp(-(15/10)**2/2)=0.32, exp(-(20/10)**2/2)=0.135 at those two points
# -- a real, steep falloff exactly where it was flagged, vs pitch_gate's
...
# (2026-08-19, user report of front-leg-only vs back-leg-only
...
# Placeholder values -- first attempt, expect a tuning pass.
...
# somewhat in sync". Investigation (in a "Bound-Gait Synchronization"
# review) recommended against ALSO rewarding the diagonal (trot-style) pairing pattern the
...
# FLOORED (not allowed to reach 0) -- guarding against the
# same "everything must be perfect simultaneously" over-gating risk
# already flagged once before (see foot_usage_gate's
...
# NOT a fixed absolute speed -- since
...
# forward"; investigation (in a "Prioritizing
# Tool Usage" review) diagnosed that a straight/locked knee during
...
# direction of the report exactly, but the SPECIFIC proposed threshold
...
# either end (the original proposal) would not have caught this at
...
# exploitation policy was happy to ride. A follow-up review proposed
# final numeric values --
# verified those proposed values against this gate's REAL formula (the
# proposal had assumed a different smooth-blend shape) before adopting: at sigma=0.12,
...
# exception below the other gates' own 0.5 floor: confirmed
# against this gate's actual formula that a 0.5 floor
...
# -- sim-to-real motivation: user wants the trained
...
# following investigation into "no_stiffness_slew_100/
# _200: still falling backward despite WALK_TARGET_PITCH_RAD" -- option
# (c) was implemented first per that recommendation): live-rollout
...
# crouch tip backward. The framing here: give the rise a mechanical
...
# bug found the same day the lean was added -- "HOME_START_
...
# option (b) from that same investigation, layered on top of HOME_START_FORWARD_LEAN_RAD
...
# reward-shaping falloff vs. a hard episode-ending cutoff). A
# worked margin (+-15deg around the new WALK_TARGET_PITCH_RAD) was used
# directly here.
WALK_PITCH_GATE_SIGMA_RAD = np.radians(15)

# WALK-only, added 2026-08-09 (user: "robot is moving forward but not
# straight" -- from an investigation titled "help WALK
...
# penalties) rather than to the diluted 3-axis term. This carries
# a real risk directly comparable to WALK_PITCH_PENALTY_WEIGHT's
...
# hardware; investigation titled "v20 still
...
# version was first implemented ("help WALK walk straight"),
...
# the framing here: the rate penalty is a damping (D) term
...
# WALK-wide -- added 2026-08-11 (following a "v27 checkpoint arc
# diagnostic + swing-time-fairness" investigation) after
...
#
# REDESIGNED 2026-08-15 (`foot_clearance_v1` diagnosis, from the
# "swing_fairness_penalty and forward lean" investigation):
...
# front_clearance_reward/back_clearance_reward (2026-08-17, from a
# "three follow-up asks... back-leg
# dragging" investigation) -- replaces the single foot_clearance_reward's all-4-leg
...
# ORIGINAL pre-split foot_clearance_reward weight exactly) --
# the reasoning: "start with equal weight...
...
# RAISED 2.0 -> 3.0 (2026-08-18, same investigation, "back-leg
...
# back legs still drag somewhat -- the decision (option a):
...
# _foot_clearance_reward()'s target_m override). 0.08m was
# chosen -- meant to give back legs real margin against the
...
# -- the reasoning (option c): introducing a stark new penalty
...
# swing_amplitude_penalty (2026-08-14, from an investigation
# proposing a new swing-AMPLITUDE-discrepancy
...
#
# FIRST DESIGN CONSIDERED (REJECTED): a RELATIVE comparison,
# max(0, back_mean_deviation -
# front_mean_deviation), penalizing only when back legs deviate from
# neutral more than front legs. Rejected because it carries the same
# class of risk as trot_symmetry_reward's own coupling problem: nothing stops
...
#
# ADOPTED INSTEAD: an ABSOLUTE, fully
...
#
# UNGATED by forward_progress, deliberately, given this session's own
# history: gating a postural term by
...
# swing_fairness_penalty's own history in this file, since
...
# calf_amplitude_penalty (2026-08-14, following investigation into
# "calf tuck persists after the thigh-amplitude fix worked").
...
# the decision was to proceed immediately with this over the
...
# documented future direction, not built now).
...
# regression).
#
# REPLACED ENTIRELY (2026-08-14, same day, after finding
# "calf_amplitude_penalty measured the WRONG
...
#
# Confirmed RAW hinge (not absolute) is the correct
...
# NOT a rolling velocity-magnitude EMA (the first instinct here) --
...
# traversed) can't be gamed that way. Decided to REPLACE this term
...
# RAISED 20deg -> 35deg (2026-08-16, from a
# "forward_tilt_walk_v1@4M checkpoint check"): measured
...
# "just barely moving" once it clears the minimum. The
# recommendation: keep this as a FLOOR, not a target/deadzone
...
# direct user request -- the alternative considered was to wait for
...
# 2026-08-12 (following a "PPO_6000000_
# jointlimit_probe_v1 real deployment" review). Was a TEMPORARY diagnostic term
...
# soft linear ramp from 0 (2026-08-10,
# following pushback during a "fix it" review): a plain linear climb_progress gate still paid out
...
# (2026-08-10, "stand, hold X seconds, THEN walk"): after
...
#
# User originally asked for a 5.0s default; further investigation
# flagged a real conflict: MAX_EPISODE_STEPS=1000 at the
...
# following "stand, hold X seconds" review):
# the first draft reused WALK_CLIMB_GATE_THRESHOLD (0.8) to also start the
# hold-timer -- the issue found: at 0.8 the policy could reach a
...
# checkpoint's score plateaued, standing but never walking -- from a
# "v3 stands but never walks" investigation). 0.95
...
# above (2026-08-10, an extension on the same v3 diagnosis):
...
# sustained standing hold overall. The suggested example value
# (0.2s) was used, not independently tuned.
...
#
# RAISED 0.5 -> 2.5 (2026-08-11, from investigation into
# "v24 plateauing" -- primary lever against a persistent
...
# The recommendation was AGAINST also tightening
...
# gait_style's own comment -- from an investigation that
# found trot's OWN cross-leg
...
# necessary here, unlike trot's case. The answer to the exact
...
# -- the concern was the EARLY-introduction risk, not that 2.5 itself is
# unsafe once the gate has let basic locomotion establish first.
#
# RAISED 2.5 -> 5.0 (2026-08-19, task #43 follow-up, from an
# investigation into "foot-contact data shows the policies
...
# front/back BOUND, despite gait_symmetry_reward being active. Diagnosis:
# the policy is stuck in a local optimum where its already-converged
...
# transition. The recommended range was 5.0-10.0; starting at the more
# conservative end.
...
# walk_policy_v11, per the trot-symmetry investigation this follows
# from. This is a real
...
# _walk_default_action_rad from tick 0 (2026-08-17, from an
# "Analysis of Issue.md" investigation and its "Rise Asymmetry
...
# a suggested 0.5-1.0s range), not swept.
...
# flagged stale ever since). Every
...
# Deliberately not changed without further verification.
...
# of an attempt to learn anything. Instead, a leg has to stay in
...
# fix this comment describes, and which notes from a 2026-08-06 "Instant
# Death on Knee Scuff" review claimed was applied, never actually landed
...
# standing/walking -- confirmed that a height gate doesn't make this
# redundant.
#
...
# Hard cutoff for _knee_walking_too_long()'s gate (2026-08-16, from the
# same review as NONTIP_TERMINATION_S's
# fix above -- the recommendation: "a hard height cutoff rather
...
# lowering it to 100.0 same day (following a "sim-to-real slew-rate
# mismatch" review) -- that change was made
...
#
# Investigation on 2026-08-07 ("MAX_SLEW_DEG_PER_S was the real root cause
# all along") found: training under the REAL 100deg/s clamp was tested
...
# 552.2), no other confound changed. Mechanistic explanation (same date):
# PPO's raw per-tick exploration noise is large and largely
...
# own. An EMA-smoothing alternative was also tried
# (`EMA_ALPHA=0.15`) and reverted -- made every metric worse.
#
# The actual, deliberate resolution (2026-08-12, following review of
# the real GO-M8010-6 motor specs): keep training under the loose 1000
...
# confirmed able to sustain 50Hz control_rate_hz -- per the question "can
# the Jetson actually sustain 50Hz control_rate_hz?") is
...
# actually be deployed under, not some sim-only approximation of it (the
# reasoning here, from a "sim-side slew-rate
# curriculum" review: allowing sim to keep a looser limit than deployment enforces
...
# EMA output-smoothing filter TRIED AND REVERTED (2026-08-11, following
# investigation titled "the alpha=0.3 fix made things worse"). Attempted as a sim-to-real
...
# more training." Conclusion: abandon the architectural
...
# TORQUE_BELT mode only (2026-08-05, user request): PD gains for the automatic
...
#
# REVERTED 3.0 -> 0.1 (2026-08-11, from investigation into
# "the triad already exists, and its timescale constants look
...
#
# REVERTED 3.0 -> 0.3 (2026-08-11, from the same investigation into
# "the triad already exists, and its timescale constants look
...
# then SUPERSEDED same day by an ASYMMETRIC split (from investigation
# into "swing_fairness_penalty and forward lean"):
...
# sustained-divergence cutoff -- described as "a cliff
...
# following a "v10 leans and tumbles,
...
class DogEnv(gym.Env):
    ...
    def __init__(self, model_path=None,
        ...
        # 'torque_belt' (2026-08-05, user request): same torque action
        # space as 'torque', but
        ...
        #
        # 'random' (2026-08-09, direct user request -- "single WALK
        # policy that stands up
        ...
        # own shape/reasoning -- see that callback's docstring).
        # Investigation into "user wants to push
        # toward faster walking" found: an INSTANT target change would drop
        ...
        # user request -- trot_home_v1/trot_stand_1ms_v1 both
        ...
        # empirically-measured per-leg signs above exactly (verified with
    ...
    def step(self, action):
        ...
        if self.control_mode == 'torque':
            ...
            # see control_mode's comment in __init__ and BELT_SERVO_KP's
            ...
            # per the question "can the Jetson actually sustain
            # 50Hz control_rate_hz?"). Sim steps at 100Hz
    ...
    def _get_rise_progress(self):
        ...
        (2026-08-16, from the "HOME_
        START_FORWARD_LEAN_RAD broke home start entirely" investigation --
        the answer to
    ...
    def _stand_progress(self):
        ...
        (from a "real hardware:
        leg gets caught" investigation -- user deployed PPO_5000000_walk_position_
    ...
    def _torso_roll_rad(self):
        ...
        from investigation into "Why the reward
    ...
    def _body_frame_xy_velocity(self):
        ...
        Added 2026-08-09 (from an investigation titled
    ...
    def _bound_symmetry_reward(self):
        ...
        CONTINUOUS since 2026-08-19 (task #43 item 2, following
        independent research into the topic): the ORIGINAL formula (this docstring's own
    ...
    def _foot_clearance_reward(self, leg_indices=None, target_m=None):
        ...
        REFORMULATED 2026-08-06: the OLD per-leg formula was a
        ...
        leg_indices (2026-08-17, from a "three follow-up asks...
        back-leg dragging" investigation): optional
        ...
        total = 0.0
        # target_m (2026-08-18, from a "back-leg clearance: user wants
        # MORE than the current split fix
        # gives" investigation): optional override of FOOT_CLEARANCE_TARGET_M, for
        ...
        # not just more pressure toward the same one, per that
        # investigation's option (a) answer). sigma scales WITH the target (same target/2
    ...
    def _update_standing_hold_time(self):
        ...
        PATIENCE-BUFFERED RESET (2026-08-10, an extension on
        the v3-plateau diagnosis, "v3 stands but never
    ...
    def _update_leg_phase_time(self):
        ...
        STAND+WALK ONLY EXEMPTION (2026-08-10, from the "v3 stands but
        never walks" investigation): while
    ...
    def _compute_reward_stand(self, action):
        ...
        # AIRBORNE PENALTY ADDED 2026-08-07 (from a "3-legged standing
        # came back" investigation -- user
    ...
    def _compute_reward_walk(self, action):
        ...
        # forward but not straight" -- from an investigation titled
        # "help WALK walk straight"): even the qvel[1] fix
        ...
        #
        # REFORMULATED 2026-08-06 (from the "v10 leans and tumbles,
        # still gains reward"
        ...
        # ROLL-GATED (2026-08-17, from the "roll_penalty tuning"
        # investigation -- diagnosis: PPO_28500000_
        ...
        # (from a "fix it" review that pushed back on the first
        # proposal): a plain linear
        ...
        # linearly 0->1 over the last 20% -- the framing:
        ...
        # from the "stand, hold X
        # seconds" investigation): the height ramp above unlocks forward credit
        ...
        # deferral -- from investigation into
        # "swing_fairness_penalty and forward lean"): flagged
        ...
        # climb_posture_reward (2026-08-09, from a "single WALK policy
        # that stands up from
        # home AND walks" investigation, RE-SCOPED 2026-08-10 -- see this section's own
        ...
        # penalizes that during the climb specifically. An
        # alternative suggestion (fully un-gate upright_reward) was
        # considered and rejected -- upright_reward is deliberately gated to prevent a
        ...
        #
        # STUCK-PHASE BRANCH LEFT UNGATED (2026-08-07, from a
        # "v11 does a split, stops walking
        ...
        # comparison (a relative formula can't stop PPO
        ...
        #
        # TWO-PHASE TARGET for is_stand_and_walk (2026-08-10, from a
        # "real hardware: leg gets
        # caught" investigation -- same root-cause fix as _stand_progress()): during the
        # HOLD phase (before self._standing_hold_time reaches
        # WALK_STAND_HOLD_S), target FULL STAND_HEIGHT_M instead of the
        # crouched _walk_target_height_m -- the framing: "stand
        ...
        # height_sag_penalty (2026-08-11, following the decision to
        # "revert EMA and implement the two
        ...
        # confirmed the mechanism directly (investigating "the whole
        ...
        # fraction 0.7 crouch experiment, same
        ...
        # (a known tradeoff flagged in the v9 era): a robot that hasn't started
        ...
        # the 'home'->standing rise (2026-08-16, option (b) from the
        # same investigation as HOME_START_FORWARD_LEAN_RAD above --
        # layered on top of that reset-pose bias, not instead of it, per
        # the recommendation to combine both). pitch_penalty
        ...
        # standing/walking reward completely untouched (per the
        # "leave steady-state behavior undistorted" condition).
        ...
        walk_reward_total = (
            ...
            # angular_vel_penalty RESTORED 0.0 -> 1.0 (2026-08-06, following
            # code review):
            ...
            # DROPPED 2026-08-11 (from the "the triad already exists"
            # investigation -- same round as the
            ...
            # The recommendation, matching the reference
            ...
            # ADDED 2026-08-11 (from a "swing-time-fairness"
            # investigation -- see WALK_SWING_FAIRNESS_
        ...
        # height_overshoot_penalty -- following a live suspicion (from
        # "two new ideas... remove the fixed-height constraint") that those
        ...
        if self.gait_style == 'bound':
            ...
            # derivation, from a
            # "thigh-ROM-parity guardrail plan" review). Two lightweight
            ...
            # CLEARLY degenerate, per a tuning
            # caution from that same review against an overly-strict
